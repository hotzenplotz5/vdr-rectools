...
if [ -f /usr/share/vdr-rectools/html_template.sh ]; then
    source /usr/share/vdr-rectools/html_template.sh
else
...
process_import() {
    ...
    if [[ $FF_STATUS -eq 0 ]] && [ -f "$STAGING_REC/joined.ts" ]; then
        ...
        # info-Datei erstellen: NFO-Daten haben Vorrang
        {
            ...
            [[ -n "$META_DESC" ]] && echo "D $META_DESC" || echo "D Importiert am $(date +"%d.%m.%Y")"
...
convert_pes2ts() {
    ...
    # Priority/Lifetime aus klassischem PES-VDR-Verzeichnisnamen extrahieren
    local PRIORITY=""
    local LIFETIME=""
    if [[ "$DIR_NAME" =~ \.([0-9]+)\.([0-9]+)\.rec$ ]]; then
        PRIORITY="${BASH_REMATCH[1]}"
        LIFETIME="${BASH_REMATCH[2]}"
    fi
    ...
    if [[ -f "$STAGING_REC/info.vdr" && ! -f "$STAGING_REC/info" ]]; then
        ...
        # Fehlende Lifetime/Priority ergänzen
        if [[ -n "$LIFETIME" ]] && ! grep -q '^L ' "$STAGING_REC/info"; then
            echo "L $LIFETIME" >> "$STAGING_REC/info"
        fi
        if [[ -n "$PRIORITY" ]] && ! grep -q '^P ' "$STAGING_REC/info"; then
            echo "P $PRIORITY" >> "$STAGING_REC/info"
        fi
    fi
...
show_status() {
    ...
    if [[ -f "$LOCK_FILE" ]]; then
        ...
        if [[ -n "$PID" ]] && kill -0 "$PID" 2>/dev/null; then
            if ps -p "$PID" -o args= 2>/dev/null | grep -q -E "vdr-rectools|ffmpeg"; then
                IS_RUNNING=1
            fi
        fi
    fi
